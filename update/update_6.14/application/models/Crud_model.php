...
class Crud_model extends CI_Model
{
   ...
   // Category Sub Category Image Delete
    ...
    public function update_water_mark()
    {
        ...
        $data['value'] = $this->input->post('water_mark_opacity');
        $this->db->where('key', 'water_mark_opacity');
        $this->db->update('frontend_settings', $data);

        $data['value'] = $this->input->post('water_mark_speed');
        $this->db->where('key', 'water_mark_speed');
        $this->db->update('frontend_settings', $data);

        $data['value'] = $this->input->post('water_mark_color');
        $this->db->where('key', 'water_mark_color');
        $this->db->update('frontend_settings', $data);
    ...
    public function get_total_duration_of_lesson_by_course_id($course_id)
{
    ...
    function get_pepople_ratings($limit = 10)
        {
            $this->db->where('rating', 5);
            $this->db->where('ratable_id IS NOT NULL', NULL, false);
            $this->db->where('ratable_type IS NOT NULL', NULL, false);
            $this->db->order_by('id', 'desc');
            $this->db->limit($limit);
            return $this->db->get('rating');
        }
    ...
    public function badges_store()
    {
        $data['title']          = html_escape($this->input->post('title'));
        $data['type']           = html_escape($this->input->post('type'));
        $data['condition_from'] = html_escape($this->input->post('condition_from'));
        $data['condition_to']   = html_escape($this->input->post('condition_to'));
        $data['description']    = html_escape($this->input->post('description'));
    
        if (!file_exists('uploads/badges')) {
            mkdir('uploads/badges', 0777, true);
        }
    
        if ($_FILES['image']['name'] != "") {
            $ext = pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION);
            $image_name = md5(rand(10000000, 20000000)) . '.' . $ext;
            $upload_path = 'uploads/badges/' . $image_name;
    ...
    public function badges_delete($badge_id = '')
    {
        $badge = $this->db->get_where('badges', ['id' => $badge_id])->row_array();
    
        if (!empty($badge)) {
            if (!empty($badge['image']) && file_exists('uploads/badges/' . $badge['image'])) {
                unlink('uploads/badges/' . $badge['image']);
            }
    ...
    {
        $badge_id = html_escape($this->input->post('badge_id'));
        ...
        // Check and update image if new one uploaded
        if (!empty($_FILES['image']['name'])) {
            $ext = pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION);
            $image_name = md5(rand(10000000, 20000000)) . '.' . $ext;
            move_uploaded_file($_FILES['image']['tmp_name'], 'uploads/badges/' . $image_name);
            $data['image'] = $image_name;
    
            // Delete old image
            $old = $this->db->get_where('badges', ['id' => $badge_id])->row_array();
            if (!empty($old['image']) && file_exists('uploads/badges/' . $old['image'])) {
                unlink('uploads/badges/' . $old['image']);
            }
        }
