...
class Api_model extends CI_Model
{
	...
	public function toggle_wishlist_items_get($user_id = "")
	{
		...
		if (!isset($_GET['course_id'])) {
			return json_encode(["error" => "course_id is required"]);
		}
		...
		// Decode wishlist safely
		$wishlists = json_decode($user_details['wishlist'], true) ?: [];
	
		// Toggle wishlist
		if (in_array($course_id, $wishlists)) {
			$wishlists = array_values(array_diff($wishlists, [$course_id]));
			$status = "removed";
		} else {
			$wishlists[] = $course_id;
			$status = "added";
		...
		// Ensure my_wishlist_get exists before calling
		if (method_exists($this, 'my_wishlist_get')) {
			$this->my_wishlist_get($user_id);
		}
