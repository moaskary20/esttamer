...
class Home extends CI_Controller
{
    ...
    public function instructor_page($instructor_id = "")
    {
        ...
        if (empty($earned_badge)) {
         ...
         $course_ids = array_column($this->db->get()->result_array(), 'id');
       ...
       //  Course Sale badge 
         ...
         if (!empty($course_ids)) {
           ...
           $sale_badge = $this->db->where('type', 'courses_sale')->where('condition_from <=', $course_sale_count)
             ->where('condition_to >=', $course_sale_count)->limit(1)->get('badges')->row_array();

             //  if no badge matched exactly
             if (empty($sale_badge)) {
             $sale_badge = $this->db->where('type', 'courses_sale')->where('condition_to <', $course_sale_count)
                 ->order_by('condition_to', 'desc')->limit(1)->get('badges')->row_array();
             }
        ...
        if (!empty($instructor_id)) {
            ...
            if (!empty($course_ids)) {
                ...
                if ($rated_count > 0) {
                    ...
                    $rating_badge = $this->db->where('type', 'courses_rating')->where('condition_from <=', $rated_count)
                        ->where('condition_to >=', $rated_count)->limit(1)->get('badges')->row_array();
           ...
           $article_badge = $this->db->where('type', 'articles')->where('condition_from <=', $blog_count)->where('condition_to >=', $blog_count)->limit(1)->get('badges')->row_array();
           if (empty($article_badge)) {
               $article_badge = $this->db->where('type', 'articles')->where('condition_to <', $blog_count)->order_by('condition_to', 'desc')->limit(1)->get('badges')->row_array();
           }
      ...
      //Gamification for  Badges
        ...
        //  Course Sale badge 
            ...
            if (!empty($course_ids)) {
                ...
                ->limit(1)->get('badges')->row_array();

                //  if no badge matched exactly
                if (empty($sale_badge)) {
                $sale_badge = $this->db->where('type', 'courses_sale')
                    ->where('condition_to <', $course_sale_count)->order_by('condition_to', 'desc')
                    ->limit(1)->get('badges')->row_array();
                }
        ...
        if (!empty($user_id)) {
            ...
            if (!empty($course_ids)) {
                ...
                if ($rated_count > 0) {
                    ...
                    if (empty($rating_badge)) {
                        $rating_badge = $this->db->where('type', 'courses_rating')
                            ->where('condition_to <', $rated_count)->order_by('condition_to', 'desc')
                            ->limit(1)->get('badges')->row_array();
                    }
                }
            }
            ...
            if (empty($course_completed_badge)) {
                $course_completed_badge = $this->db->where('type', 'course_completed')
                    ->where('condition_to <', $progress_count)->order_by('condition_to', 'desc')
                    ->limit(1)->get('badges')->row_array();
            }
